...
def launchBackupForItem(bitem, key):
	...
	command = "rsync -vahz --compress-level=9 --no-devices --no-p " \
				+ "--progress " \
				+ "--stats " \
		...
		for bitem in list(ongoingBackups):
			...
			if result:
				if ongoingBackups[bitem]['tryCounter'] == ntries:
					miLogger.error("I've tried " + str(ongoingBackups[bitem]['tryCounter']) + " times to run backup for " + bitem + ", " + config['backups'][bitem]['name'] + ", resulting in ERROR")
				...
				miLogger.info("Successful backup for " + config['backups'][bitem]['name'] + ", after trying it " + str(ongoingBackups[bitem]['tryCounter']) + " times")
				ongoingBackups.pop(bitem, None)
...
def sendCompletionSignals(signalCompletion):
	...
	for bitem in signalCompletion:
		...
		for command in commands:
			...
			ongoingSignals[bitem] = {'command': command, 'tryCounter': 1, 'process': subprocess.Popen(command, shell=True)}
		...
		for bitem in list(ongoingSignals):
			...
			if result:
				if ongoingSignals[bitem]['tryCounter'] == ntries:
					miLogger.error("I've tried " + str(ongoingSignals[bitem]['tryCounter']) + " times to send the following signal " + ongoingSignals[bitem]['command'] + " for '" + config['backups'][bitem]['name'] + "', resulting in ERROR")
				...
				miLogger.info("Successful signal sent for " + config['backups'][bitem]['name'] + ", after trying it " + str(ongoingSignals[bitem]['tryCounter']) + " times")
				ongoingSignals.pop(bitem, None)
